...
def test_content_page_state_average_dedupes_hospital_and_procedure_rows():
    _db._connection = None
    _db.init_db()
    upsert_hospital_row(
        {
            "facility_id": "22901",
            "name": "Duplicate State Avg Hospital",
            "city": "Dallas",
            "state": "TX",
            "slug": "duplicate-state-avg-hospital-dallas",
            "lat": 32.77,
            "lon": -96.79,
        }
    )
    with get_db() as db:
        ...
        db.execute(
            ...
            ("022901", "70551", "MRI BRAIN W/O CONTRAST", 2000.0, 200.0, 10.0, 2026, "hospital"),
        )
        db.execute(
            "INSERT OR REPLACE INTO procedure_prices (facility_id, cpt_code, description, gross_charge, medicare_rate, markup_vs_medicare, data_year, facility_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            ("022901", "70551", "MRI BRAIN W/O CONTRAST", 2000.0, 200.0, 10.0, 2026, "hospital"),
        )
