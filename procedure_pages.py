...
def get_providers_near_zip_for_cpt(
    ...
    # If no providers are available in-radius, return nearest same-state matches
    # (distance-ranked) so users still get useful options without crossing states.
    fallback = _get_nearest_same_state_providers_for_cpt(
        cpt_code=cpt_code,
        zip_code=zip_code,
        zip_lat=zip_lat,
        zip_lon=zip_lon,
...
def _get_nearest_same_state_providers_for_cpt(
    cpt_code: str,
    zip_code: str,
    zip_lat: float,
    zip_lon: float,
    ...
    # Prefer nearest options first; tie-break by configured sort.
    if sort_by == "markup":
        out.sort(key=lambda r: (r["distance_miles"], r["markup_vs_medicare"] is None, r["markup_vs_medicare"] or 0.0))
    else:
        out.sort(key=lambda r: (r["distance_miles"], r["estimated_patient_cost"] is None, r["estimated_patient_cost"] or 0.0))
    return out[:limit]
